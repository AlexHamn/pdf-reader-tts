...
export function ChatPanel({ documentId, isReady, embedded = false }: ChatPanelProps) {
  ...
  if (!isReady) {
    ...
    if (embedded) {
      return <div className="flex-1 flex items-center justify-center p-4">{loadingContent}</div>;
    }

    return (
      <Card className="border-dashed">
        <CardContent className="py-8">{loadingContent}</CardContent>
      </Card>
    );
  }
  ...
  // Embedded mode - for use inside Sheet or other containers
  if (embedded) {
    return (
